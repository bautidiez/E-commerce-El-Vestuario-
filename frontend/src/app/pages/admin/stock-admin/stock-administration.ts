...
export class StockAdminComponent implements OnInit, OnDestroy {
    ...
    this.apiService.getCategorias(false, undefined, true).subscribe({
      next: (data: any) => {
        ...
        this.categorias = data.map((cat: any) => ({
          ...cat,
          id: Number(cat.id),
          nombre_formateado: this.getFlattenedCategoryPath(cat, data)
  ...
  private getFlattenedCategoryPath(cat: any, allCats: any[]): string {
    if (!cat.categoria_padre_id) return `🏠 ${cat.nombre}`;
    const padre = allCats.find(c => Number(c.id) === Number(cat.categoria_padre_id));
    if (padre) return `${this.getFlattenedCategoryPath(padre, allCats)} > ${cat.nombre}`;
    return cat.nombre;
  }
